...
function MyPlanTab({ userId }: { userId: string | null }) {
  ...
  // Load villages for place dropdown
  useEffect(() => {
    fetch('/api/masters/villages').then(r => r.json()).then(d => {
      if (Array.isArray(d)) setVillages(d.map((v: { id: string; name: string }) => ({ id: v.id, name: v.name })))
    }).catch(() => {})
  }, [])
